...
async def get_message(message):
    ...
    if message.text == "🌯 Подбор шаурмичных":
        ...
        if latitude > 0: 
            ...
            for i in range(len(result)):
                ...
                if (abs(latitude - float(coordinates[0])) < 0.0045) or (abs(float(coordinates[0]) - latitude) < 0.0045) or (abs(longitude - float(coordinates[0])) < 0.0045) or (abs(float(coordinates[0]) - longitude) < 0.0045):
                    shawa_cnt += 1
                    str_out += f'🌯Название: {date[1]}. Адрес: {date[3]}. Рейтинг: {date[6]}★ Внутренний номер (для просмотра отзывов): {date[0]}🌯 \n'
            if shawa_cnt > 0:
                await bot.send_message(message.chat.id, f'{str_out}')
            else:
                await bot.send_message(message.chat.id, "К сожалению, в радиусе 1км шаурмичных из базы нашего бота не обнаружено")
        else: 
    ...
    if message.text == "🧑 Личный кабинет":
        ...
        cur = conn.cursor()
        prem = 0        
        cur.execute(f'SELECT * FROM premium WHERE user_id = "{message.chat.id}" AND status = "Одобрено"')
        result = cur.fetchall()
        if len(result) > 0:
            prem = 1
        cur.execute(f'SELECT * FROM users WHERE user_id = "{message.chat.id}"')
    ...
    if "/add" in message.text:
        ...
        if len(message_inform) > 1: 
            ...
            if len(result) > 0:
                premium = 1
            if premium == 1:
                cur.execute(f'INSERT INTO reviews (name, vn_nom, score, review, user_id) VALUES ("[PREMIUM] {message.from_user.first_name}", "{vn_nom}", "{score}", "{review}", "{message.chat.id}")')
                result = cur.fetchall()
                print(result)
                conn.commit()
            else:
