...
async def get_message(message):
    ...
    if message.text == "🌯 Рейтинг шаурмичных":
        ...
        for i in range(len(result)):
            date = result[i]       
            str_out += f'🌯Название: {date[1]}. Адрес: {date[3]}. Рейтинг: {date[6]}★ Внутренний номер (для просмотра отзывов): {date[7]}🌯 \n'
        print(str_out)
    ...
    if message.text == "🌯 Подбор шаурмичных":
        ...
        if latitude > 0: 
            ...
            for i in range(len(result)):
                ...
                if (abs(latitude - float(coordinates[0])) < 0.02) or (abs(float(coordinates[0]) - latitude) < 0.02) or (abs(longitude - float(coordinates[0])) < 0.02) or (abs(float(coordinates[0]) - longitude) < 0.02):
                    shawa_cnt += 1
                    str_out += f'🌯Название: {date[1]}. Адрес: {date[3]}. Рейтинг: {date[6]}★ Внутренний номер (для просмотра отзывов): {date[7]}🌯 \n'
            if shawa_cnt > 0:
                await bot.send_message(message.chat.id, f'{str_out}')
